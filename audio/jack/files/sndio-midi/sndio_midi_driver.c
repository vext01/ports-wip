...
sndio_midi_driver_new (jack_client_t *client, const char *name)
{
	...
	// hopefully not needed XXX
	//driver->midi   = sndio_seqmidi_new(client, NULL);
...
#define SNDIO_MIDI_NAME		"sndio_midi"
	strncpy (desc->name, SNDIO_MIDI_NAME, sizeof(SNDIO_MIDI_NAME));
	desc->nparams = 0;
